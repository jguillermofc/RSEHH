...
def obtainMinAndMax(problem, m):
    if problem in ['DTLZ1']:
        zmin = np.zeros(m)
        zmax = np.ones(m)*(0.5)
    elif problem in ['DTLZ2', 'DTLZ3', 'DTLZ4', 'IMOP1', 'IMOP2', 'IMOP4', 'IMOP6', 'IMOP7']:
        zmin = np.zeros(m)
        zmax = np.ones(m)
    elif problem in ['DTLZ5']:
        zmin = np.zeros(m)
        if m == 3:
        ...
        zmax = np.zeros(m)
    elif problem in ['DTLZ2_MINUS', 'DTLZ4_MINUS']:
        zmin = np.ones(m)*(-3.5)
        zmax = np.zeros(m)
    elif problem in ['DTLZ3_MINUS']:
        zmin = np.ones(m)*(-2.203603e+03)
        zmax = np.zeros(m)
    elif problem in ['DTLZ5_MINUS']:
        zmax = np.zeros(m)
        if m == 3:
    ...
    elif problem in ['WFG1', 'WFG2', 'WFG4', 'WFG5', 'WFG6', 'WFG7', 'WFG8', 'WFG9']:
        zmin = np.zeros(m)
        zmax = np.arange(2.0, 2*m+1, 2)
    elif problem in ['WFG3']:
        zmin = np.zeros(m)
        if m == 3:
